                  ...
                  {data?.response?.items?.map((item, index) => (
                    item ? (
                      <tr key={index} class="border-b border-gray-200 dark:border-gray-700">
                        ...
                        <th scope="row" class=" text-blue-950  dark:text-white font-medium text-lg text-center align-text-top py-3 px-3 whitespace-nowrap">
                        ...
                        <td class="list-none py-2.5 dark:text-white text-blue-950">
                          ...
                          <li class="px-6 py-1  grid">
                            <Link to={`/match-detail/${item.match_id}/${formatUrl(`${item.short_title}-${item.competition.title}`)}/commentary`} className=' md:w-full w-72 items-center dark:bg-gray-700 grid grid-rows-2 gap-y-2 px-3 py-2 justify-start bg-gray-200'>
